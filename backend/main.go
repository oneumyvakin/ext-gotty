...
func main() {
	...
	portRangeStop := flag.Int("port-range-start", 10000, "Port range stop")
	...
	if *gottyPath != "" {
		...
		go func() {
			result := runResult{}
			result.output, result.outputBytes, result.code, result.err = execute(*gottyPath, cmdArgs...)
...
func acquirePort(portChan int, portRangeStart int, portRangeStop int) {
    var listener net.Listener
    var err error
    var port int
    for port = portRangeStart; port <= portRangeStop; port++ {
        listener, err = reuseport.NewReusablePortListener("tcp", fmt.Sprintf(":%d", port))
        if err != nil {
            continue
        }
    ...
    if err != nil {
        log.Fatal("All TCP ports in range are busy")
    }
